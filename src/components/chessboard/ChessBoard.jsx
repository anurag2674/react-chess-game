    ...
    Object.keys(pieces)?.map((sinlgePiece) => {
      if (pieces[sinlgePiece]?.color !== currentPiece?.color) {
        if (pieces[sinlgePiece]?.type !== 'king') {
          concatedArray = [
            ...concatedArray,
            ...checkMovesForSinglePiece(pieces[sinlgePiece], sinlgePiece[0], sinlgePiece[1], pieces),
          ];
        }
      }
    });
  ...
  function castling() {
    ...
    if (
      ...
      if (
        ...
        (pieces[firstSelected.current]?.type === 'rook' && firstSelected.current === '81') ||
        (pieces[secondSelected]?.type === 'rook' && secondSelected === '88') ||
        (pieces[firstSelected.current]?.type === 'rook' && firstSelected.current === '88')
      ) {
    ...
    if (
      ...
      if (
        ...
        (pieces[firstSelected.current]?.type === 'rook' && firstSelected.current === '11') ||
        (pieces[secondSelected]?.type === 'rook' && secondSelected === '18') ||
        (pieces[firstSelected.current]?.type === 'rook' && firstSelected.current === '18')
      ) {
        if (isCheckMate.black === false && isCheckMate.white === false) {
    ...
    <div className="container">
      {isGameOver && (
        <div className="gameOver">
          ...
          <br />
          <p className="winner">{isCheckMate.black === true ? 'White' : 'Black'} Won</p>
        </div>
      )}
      {couldBePromoted && (
        <Promotion couldBePromoted={setCouldBePromoted} promotionType={setPromotionType} pieceColor={color} />
      )}
