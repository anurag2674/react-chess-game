    ...
    if (flagUp) {
      if (!checkIfPieceRightLeftPinnedToKing(thePiece, currentCol, currentRow, pieces)) {
        if (pieces[currentCol + rookNewPosition(currentRow, '+', i)] === undefined) {
          ...
          pieces[currentCol + rookNewPosition(currentRow, '+', i)]?.type === 'king' &&
          pieces[currentCol + rookNewPosition(currentRow, '+', i)]?.color !== thePiece.color
        ) {
          allowedMoves.push(currentCol + rookNewPosition(currentRow, '+', i));
          flagUp = false;
        } else if (
          pieces[currentCol + rookNewPosition(currentRow, '+', i)] !== undefined &&
          pieces[currentCol + rookNewPosition(currentRow, '+', i)].type !== 'king'
        ) {
          allowedMoves.push(currentCol + rookNewPosition(currentRow, '+', i));
          flagUp = false;
    ...
    if (flagDown) {
      if (!checkIfPieceRightLeftPinnedToKing(thePiece, currentCol, currentRow, pieces)) {
        if (pieces[currentCol + rookNewPosition(currentRow, '-', i)] === undefined) {
          ...
          pieces[currentCol + rookNewPosition(currentRow, '-', i)]?.color !== undefined &&
          pieces[currentCol + rookNewPosition(currentRow, '-', i)]?.type !== 'king'
        ) {
          allowedMoves.push(currentCol + rookNewPosition(currentRow, '-', i));
          flagDown = false;
    ...
    if (flagRight) {
      if (!checkIfPieceTopDownPinnedToKing(thePiece, currentCol, currentRow, pieces)) {
        if (pieces[rookNewPosition(currentCol, '+', i) + currentRow] === undefined) {
          ...
          pieces[rookNewPosition(currentCol, '+', i) + currentRow]?.type === 'king' &&
          pieces[rookNewPosition(currentCol, '+', i) + currentRow]?.color !== thePiece.color
        ) {
          allowedMoves.push(rookNewPosition(currentCol, '+', i) + currentRow);
          flagRight = false;
        } else if (
          pieces[rookNewPosition(currentCol, '+', i) + currentRow]?.color !== undefined &&
          pieces[rookNewPosition(currentCol, '+', i) + currentRow]?.type !== 'king'
        ) {
          allowedMoves.push(rookNewPosition(currentCol, '+', i) + currentRow);
          flagRight = false;
