...
export function getFriedlyPiecesAllowedMoves(pieces, enemyColor, attackerCurrentSquare, defensableSquares) {
  ...
  Object.keys(pieces)?.map((sinlgePiece) => {
    if (pieces[sinlgePiece]?.color !== enemyColor) {
      ...
      let singlePieceAllowedMoves = checkMovesForSinglePiece(
        pieces[sinlgePiece],
        sinlgePiece[0],
        sinlgePiece[1],
        pieces
      ...
      if (singlePieceAllowedMoves.includes(attackerCurrentSquare)) {
        if (blockDefenders.hasOwnProperty([pieces[sinlgePiece]?.type + sinlgePiece])) {
          ...
          blockDefenders = {
            ...blockDefenders,
            [pieces[sinlgePiece]?.type + sinlgePiece]: [
              blockDefenders[[pieces[sinlgePiece]?.type + sinlgePiece]],
  ...
  let filteredArray = concatedArrayTow?.filter(
    (item) => !item.includes('0') && !item.includes('9') && item.length <= 2
  );
  return filteredArray;
}

export const AllowedMovesToEscapeCheckMate = (
  isCheckMate,
  attackerPiece,
  attackerCurrentSquare,
  currentKingSquare,
  ...
  kingPossibleMoves.map((kingPossibleMove) => {
    if (
      !enemyPiecesAllowedMoves.includes(kingPossibleMove) &&
  ...
  if (checkMateType === 'double') {
    ...
    let finalArray = Array.from(new Set(kingAllowedMoves)).filter(
      (item) => item !== oppositeSquare && !item.includes('0') && !item.includes('-') && !item.includes('9')
    );
  ...
  let filterdKingMoves = kingAllowedMoves.filter(
    (item) => !item.includes('9') && !item.includes('0') && !item.includes('-') && item !== oppositeSquare
  );
