...
function handleKingNormalMoves(currentCol, currentRow, pieces) {
  ...
  allowedMoves = kingPossibleMoves.filter(
    (move) =>
      !enemyPiecesAllowedMoves.includes(move) &&
      move.length <= 2 &&
      !move.includes('0') &&
      !move.includes('9') &&
      move !== currentCol + currentRow
  );
  return allowedMoves;
}
  ...
  if (pieces) {
    for (let i = 1; i <= 8; i++) {
      ...
      switch (i) {
        //
        case 1:
          possibleSquare = col + (rowNum + 1).toString();
        ...
        case 2:
          possibleSquare = colNum.toString() + (rowNum - 1).toString();
          if (pieces[possibleSquare]?.color !== kingColor) {
            kingPossibleMoves.push(possibleSquare);
          }
          break;
        ...
        case 3:
          possibleSquare = (colNum - 1).toString() + rowNum;
          if (pieces[possibleSquare]?.color !== kingColor) {
            kingPossibleMoves.push(possibleSquare);
          }
          break;
        ...
        //
        case 5:
          possibleSquare = (colNum - 1).toString() + (rowNum + 1).toString();
          if (pieces[possibleSquare]?.color !== kingColor) {
            kingPossibleMoves.push(possibleSquare);
          }
          break;
  ...
  Object.keys(pieces)?.map((sinlgePiece) => {
    if (pieces[sinlgePiece]?.color === enemyColor) {
      ...
      if (pieces[sinlgePiece]?.type !== 'king') {
        concatedArray = [
          ...concatedArray,
          ...checkMovesForSinglePiece(pieces[sinlgePiece], sinlgePiece[0], sinlgePiece[1], pieces, chekker),
        ];
      }
    }
  });
  ...
  switch (thePiece?.type) {
    case 'pawn':
      ...
      if (pieces[currentCol + handleNewPosition(currentRow, '1', thePiece.color)] === undefined) {
        if (
          thePiece.basePostion === true &&
          pieces[currentCol + handleNewPosition(currentRow, '2', thePiece.color)] === undefined
        ) {
          if (chekker !== 'king') {
            ...
            !checkIfPieceDiagonallyLeftPinnedToKing(thePiece, currentCol, currentRow, pieces)
              ? allowedMoves.push(
                  currentCol + handleNewPosition(currentRow, '2', thePiece.color),
      ...
      if (thePiece?.color === 'white') {
        ...
        if (
          (pieces[handleAttackPosition(currentCol, '1', '+') + handleAttackPosition(currentRow, '1', '+')] !==
            undefined &&
            pieces[handleAttackPosition(currentCol, '1', '+') + handleAttackPosition(currentRow, '1', '+')]?.type !==
              'king') ||
          (chekker === 'king' &&
            pieces[handleAttackPosition(currentCol, '1', '+') + handleAttackPosition(currentRow, '1', '+')]?.color !==
              'white')
        ) {
          allowedMoves.push(
            !checkIfPieceDiagonallyLeftPinnedToKing(thePiece, currentCol, currentRow, pieces)
              ? handleAttackPosition(currentCol, '1', '+') + handleAttackPosition(currentRow, '1', '+')
              : ''
          );
        }
        if (
    ...
    case 'queen':
      allowedMoves = [];
      checkIfPieceTopDownPinnedToKing(thePiece, currentCol, currentRow, pieces)
        ? (allowedMoves = handleRookLogic(currentCol, currentRow, pieces, thePiece))
        : (allowedMoves = handleBishopLogic(currentCol, currentRow, pieces, thePiece).concat(
            handleRookLogic(currentCol, currentRow, pieces, thePiece)
          ));
